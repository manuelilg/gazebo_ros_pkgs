...
#include "gazebo_plugins/ElevatorPluginPrivate.hh"
...
//  this->dataPtr->doorController = NULL;
  this->dataPtr->doorControllers = new std::vector<ElevatorPluginPrivate::DoorController*>;
...
//  this->dataPtr->doorController = NULL;
  for (auto doorCtrl : *this->dataPtr->doorControllers) {
    delete doorCtrl;
  }
  delete this->dataPtr->doorControllers;
...
void ElevatorPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  // Get the door joint
  std::string doorJointNames =
    this->dataPtr->sdf->Get<std::string>("door_joint");

  boost::erase_all(doorJointNames, " ");
  std::vector<std::string> doorJointNames_;
  boost::split(doorJointNames_, doorJointNames, boost::is_any_of(","));

  for(std::string doorJointName : doorJointNames_)
  {
//    this->dataPtr->doorJoint = this->dataPtr->model->GetJoint(doorJointName);
    physics::JointPtr doorJoint = this->dataPtr->model->GetJoint(doorJointName);

    if (doorJoint)
    {
      this->dataPtr->doorJoints.push_back(doorJoint);
    }
    else
    {
...
//  }
  ...
  float doorJointMoveDistance = 1.0;
  // Get the distance for the door joint(s)
  if (this->dataPtr->sdf->HasElement("door_joint_move_distance"))
  {
    doorJointMoveDistance = this->dataPtr->sdf->Get<double>("door_joint_move_distance");
  }
  gzmsg << "doorJointMoveDistance set to: " << doorJointMoveDistance << std::endl;
  this->dataPtr->doorJointMoveDistance = doorJointMoveDistance;

  // Create the door and lift controllers.
  for (physics::JointPtr doorJoint : this->dataPtr->doorJoints) {
    this->dataPtr->doorControllers->push_back(new ElevatorPluginPrivate::DoorController(
      doorJoint, this->dataPtr->doorJointMoveDistance));
  }
//  this->dataPtr->doorController = new ElevatorPluginPrivate::DoorController(
//      this->dataPtr->doorJoints.at(0), this->dataPtr->doorJointMoveDistance);
  ...
  gzmsg << "Modified ElevatorPlugin loaded\n";
}
...
{
  for (auto ctr: *this->doorControllers) {
    delete ctr;
  }
  delete this->doorControllers;
...
/////////////////////////////////////////////////
ElevatorPluginPrivate::CloseState::CloseState(std::vector<DoorController*> *_ctrl)
  : State(), ctrl(_ctrl)
{
}
...
{
  for (DoorController* doorCtrl : *this->ctrl) {
    doorCtrl-> SetTarget(ElevatorPluginPrivate::DoorController::CLOSE);
  }
...
bool ElevatorPluginPrivate::CloseState::Update()
{
  ...
  else
  {
    bool rVal = true;
    for (DoorController* doorCtrl : *this->ctrl) {
      rVal = rVal & (doorCtrl->GetTarget() == ElevatorPluginPrivate::DoorController::CLOSE &&
                  doorCtrl->GetState() == ElevatorPluginPrivate::DoorController::STATIONARY);
    }
    return rVal;
...
/////////////////////////////////////////////////
ElevatorPluginPrivate::OpenState::OpenState(std::vector<DoorController*> *_ctrl)
  : State(), ctrl(_ctrl)
{
}
...
{
  for (DoorController* doorCtrl : *this->ctrl) {
    doorCtrl->SetTarget(ElevatorPluginPrivate::DoorController::OPEN);
  }
...
bool ElevatorPluginPrivate::OpenState::Update()
{
  ...
  else
  {
    bool rVal = true;
    for (DoorController* doorCtrl : *this->ctrl) {
      rVal = rVal & (doorCtrl->GetTarget() == ElevatorPluginPrivate::DoorController::OPEN &&
                  doorCtrl->GetState() == ElevatorPluginPrivate::DoorController::STATIONARY);
    }
    return rVal;
//    return this->ctrl->GetTarget() ==
...
/////////////////////////////////////////////////
ElevatorPluginPrivate::DoorController::DoorController(physics::JointPtr _doorJoint, float moveDistance)
  : doorJoint(_doorJoint), state(STATIONARY), target(CLOSE), moveDistance(moveDistance)
{
    ...
    const common::UpdateInfo &_info)
{
  ...
  double errorTarget = this->target == OPEN ? this->moveDistance : 0.0;
...
{
  this->liftPID.Init(100000, 3000, 100000.0, 0.0, -800000.0);
}
